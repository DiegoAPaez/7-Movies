import { Link } from "react-router-dom";
import { useForm } from "../../hooks/useForm";
import {
    useGoogleSignInMutation,
    useLoginMutation,
} from "../query/authQueries";

export const LoginForm = () => {
    const initialForm = { email: "", password: "" };
    const { onInputChange, email, password } = useForm(initialForm);
    const { mutateAsync, isPending, error } = useLoginMutation();
    const {
        mutateAsync: googleSignIn,
        isPending: isGooglePending,
        error: googleError,
    } = useGoogleSignInMutation();

    const handleSubmit = async (event: React.FormEvent) => {
        event.preventDefault();
        await mutateAsync({ email, password });
    };

    const handleGoogleSignIn = async (event: React.MouseEvent) => {
        event.preventDefault();
        await googleSignIn();
    };

    const isAnyPending = isPending || isGooglePending;
    const displayError = error ?? googleError;

    return (
        <>
            <h2 className="text-sky-950 text-center w-10/12 md:w-1/2">
                In order to use the editing and rating capabilities of Movie DB,
                as well as get personal recommendations you will need to login
                to your account. If you do not have an account, registering for
                an account is free and simple.
            </h2>
            <hr />
            <form
                className={"flex flex-col items-center justify-center w-full"}
                onSubmit={(event) => {
                    void handleSubmit(event);
                }}
            >
                <div className={"flex flex-col gap-4 mb-4 w-3/4 md:w-1/4"}>
                    <input
                        className={
                            "p-2 border border-sky-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-sky-500"
                        }
                        type="email"
                        placeholder="Email"
                        name="email"
                        value={email}
                        onChange={onInputChange}
                        required
                    />
                    <input
                        className={
                            "p-2 border border-sky-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-sky-500"
                        }
                        type="password"
                        placeholder="Password"
                        name="password"
                        value={password}
                        onChange={onInputChange}
                        required
                    />
                </div>
                <div className="flex flex-row gap-4">
                    <button
                        className="bg-sky-950 text-white font-bold py-2 px-4 rounded-lg hover:cursor-pointer hover:bg-sky-700 transition-colors duration-300"
                        disabled={isPending}
                    >
                        {isPending ? "Logging in..." : "Login"}
                    </button>
                </div>
                {displayError && (
                    <div className="text-red-500 mt-2">
                        {displayError.message}
                    </div>
                )}
            </form>
            <div className="max-w-11/12 w-lg flex flex-col gap-3 mt-3 text-sky-950">
                <p className="text-center text-sm">
                    Don't have an account?{" "}
                    <Link
                        to={"/register"}
                        className="hover:underline font-bold"
                    >
                        Sign up
                    </Link>
                </p>
                <div className="flex flex-row text-center w-full">
                    <div className="border-b-2 mb-2.5 mr-2 w-full"></div>
                    <div className="text-sm font-bold w-fit">OR</div>
                    <div className="border-b-2 mb-2.5 ml-2 w-full"></div>
                </div>
                <button
                    disabled={isAnyPending}
                    onClick={(e) => {
                        void handleGoogleSignIn(e);
                    }}
                    className={`w-full flex items-center justify-center gap-x-3 py-2.5 border rounded-lg text-sm font-medium cursor-pointer  ${
                        isAnyPending
                            ? "cursor-not-allowed"
                            : "hover:bg-sky-100 transition duration-300 active:bg-sky-100"
                    }`}
                >
                    <svg
                        className="w-5 h-5"
                        viewBox="0 0 48 48"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                    >
                        <g clipPath="url(#clip0_17_40)">
                            <path
                                d="M47.532 24.5528C47.532 22.9214 47.3997 21.2811 47.1175 19.6761H24.48V28.9181H37.4434C36.9055 31.8988 35.177 34.5356 32.6461 36.2111V42.2078H40.3801C44.9217 38.0278 47.532 31.8547 47.532 24.5528Z"
                                fill="#4285F4"
                            />
                            <path
                                d="M24.48 48.0016C30.9529 48.0016 36.4116 45.8764 40.3888 42.2078L32.6549 36.2111C30.5031 37.675 27.7252 38.5039 24.4888 38.5039C18.2275 38.5039 12.9187 34.2798 11.0139 28.6006H3.03296V34.7825C7.10718 42.8868 15.4056 48.0016 24.48 48.0016Z"
                                fill="#34A853"
                            />
                            <path
                                d="M11.0051 28.6006C9.99973 25.6199 9.99973 22.3922 11.0051 19.4115V13.2296H3.03298C-0.371021 20.0112 -0.371021 28.0009 3.03298 34.7825L11.0051 28.6006Z"
                                fill="#FBBC04"
                            />
                            <path
                                d="M24.48 9.49932C27.9016 9.44641 31.2086 10.7339 33.6866 13.0973L40.5387 6.24523C36.2 2.17101 30.4414 -0.068932 24.48 0.00161733C15.4055 0.00161733 7.10718 5.11644 3.03296 13.2296L11.005 19.4115C12.901 13.7235 18.2187 9.49932 24.48 9.49932Z"
                                fill="#EA4335"
                            />
                        </g>
                        <defs>
                            <clipPath id="clip0_17_40">
                                <rect width="48" height="48" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                    {isAnyPending ? "Signing In..." : "Continue with Google"}
                </button>
            </div>
        </>
    );
};
